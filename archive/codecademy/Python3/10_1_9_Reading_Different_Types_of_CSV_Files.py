...
# true that CSV stand for that, but it's also true that other ways of separating
# values are valid CSV files these days.
#
# People used to call Tab-Separated Values files for TSV files, but as other 
# separators grew in popularity everyone realized that creating a new .[a-z]sv 
# file format for every value-separating character used is not sustainable.
#
# So we call all files with a list of different values a CSV file and then use 
# different delimiters (like comma or tab) to indicate where the different values
# start and stop.
#
...
#
# Also notice that many of these addresses have commas in them! This is okay, we'll
# still be able to read it. If we wanted to, say, print out all the addresses in 
...
with open('addresses.csv', newline='') as addresses_csv:
	...
	for row in address_reader:
		print(row['Address'])
#
# Notice that when we call csv.DictReader we pass in the delimiter parameter, which
# is the string that's used to delineate separte fields in the CSV. We then iterate
# through the CSV and print out each of the addresses. 

# Tasks
# 1. Import the CSV module (already done in the example since i run that as well)
#
# 2. Open up the file books.csv in the variable books_csv
#
# 3. Create a DictReader instance that uses the @ symbol as a delimiter to 
# 	 read books_csv. Save the result in a variable called books_reader.
#
# 4. Create a list called isbn_list, iterate through books_reader to get the
# 	 ISBN number of every book in the CSV file. Use the ['ISBN'] key for the 
