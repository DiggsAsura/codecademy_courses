 ...
 *
 * Another useful feature of match arms is that they can bind to the parts fo the values that match
 ...
 * 2009, the United States minted quarters with different designs for each of the 50 states on one
 * side. No other coins got state designs, so only quarters have this extra value. We can add this
 * information to our enum by changing the Qurater variant to include the UsState value stored
 * inside it, which we've done here in: */
 ...
 * value UsState::Alaska. We can then use that binding in the println!( expression, thus getting
...
fn main() {
    let alaska = value_in_cents(Coin::Quarter(UsState::Alabama));
    println!("Alabama: {}", alaska);
}
