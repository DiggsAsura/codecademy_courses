 ...
 * dice roll, your player does not move, but instead gets a new fancy hat. If you roll a 7, your
 * player loses a fancy hat. For all other values, your player moves that number of spaces on the
 * game board. Here's a match that implements that logic, with the result of the dice roll
 * hardcoded rather than a random value, and all other logic represented by functions without
 ...
 * covers every other possible value, the pattern is the variable we've chosen to name other. The
 * code that runs for the other arm uses the variable by passing it to the move_player function.
 *
 * This code compiles, even though we haven't listed all the possibie values a u8 can have, because
 ...
 *
 * Rust also has a pattern we can use when we want to catch-all but don't want to use the value in
 ...
 * variable.
 *
 * Let's change the rules of the game: now, if you roll anything other than a 3 or a 7, you must
 * roll again. We now longer need to use the catch-all value, so we can change our code to use _
 ...
 * turn if you roll anything other than a 3 or 7. We can express that by using the unit value (the
