...
// variants to see which variant needs the most space. Rust sees that Message::Quit doesn't need
// any space, Message:Move needs enough space to store two i32 values, and so forth. Because only
// one variant will be used, the most space a Message value will need is the space it would take to
