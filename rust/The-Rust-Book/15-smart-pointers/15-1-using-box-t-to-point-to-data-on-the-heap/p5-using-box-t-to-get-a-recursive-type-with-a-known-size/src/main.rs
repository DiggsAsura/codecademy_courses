...
// next List value that will be on the heap rather than inside the Cons variant. Conceptually, we
// sill have a list, created with lists holding other lists, but this implementation is now more
...
// Nil variant stores no values, so it needs less space than the Cons variant. We now know that any
// List value will take up the size of an i32 plus the size of a box's pointer data. By using a
// box, we've broken the infinite, recursive chain, so the compiler can figure out the size it
// needs to store a List value. Figure 15-2 whos what the Cons variant looks like now.
//
...
// capabilities, like those we'll see with the other smart pointer types. They aslo don't have the
// performance overhead that these special capabilities incur, so they can be useful in cases like
// the cons list where the indirection is the only feature we need. We'll look at more use cases
// for boxes in Chapter 17, too.
//
// The Box<T> type is a smart pointer beacuse it implements the Deref trait, which allows Box<T>
// values to be treated like references. When a Box<T> value goes out of scope, the heap data that
// the box is pointing to is cleand up as well because of the Drop trait implementation. These two
// traits will be even more important to the functionality provided by the other smart pointer
