...
// to another type. For example, deref coercion can convert &String to &str because String
// implements the Deref trait such that it returns &str. Deref coercion is a convenience Rust
// performs on arguments to functions and methods, and works only on types that implement the Deref
// trait. It happens automatically when we pass a reference to a particular type's value as an
// argument to a function or method that doesn't match the parameter type in the function or method
// definition. A sequence of calls to the deref method converts the type we provided into the type
// the parameter needs.
//
// Deref coercion was added to Rust so that progrmamers writing function and method calls don't
...
// 15-11: A hello function that has the parameter name of type &str
...
// MyBox<String>, as shown in Listgin 15-12:
...
fn main() {
    ...
    // 15-13
    let msg = MyBox::new(String::from("Kenneth"));
    hello(&(*msg)[..]);
    // 15-13: The code we would have to write if Rust didnt't have deref coercion
}

// 15-12: Calling hello with a reference to a MyBox<String> value, which works because of deref
// coercion
...
//
// If Rust didn't implement deref coercion, we would have tro write the code in Listing 15-13
// instead of the code in Listing 15-12 to call hello with a value of type &MyBox<String>.

// The (*m) dereference the MyBox<String> into a String. Then the & and [..] take a string slice of
// the STring that is equal to the whole string to match the signature for hello. This code without
// deref coercions is harder to read, write, and understand with all of these symbols involved.
