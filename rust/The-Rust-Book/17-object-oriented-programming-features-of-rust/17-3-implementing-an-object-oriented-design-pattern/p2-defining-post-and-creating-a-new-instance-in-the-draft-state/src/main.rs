...
// private State trait that will define the behavior that all state objects for a Post must have.
//
// Then Post will hold a trait object of Box<dyn State> inside an Option<T> in a private field
// named state to hold the state object. You'll see why the Option<t> is necessary in a bit.
...
fn main() {}
// Listing 17-12: Definition of a Post struct and a new function that creates a new Post instance,
// a State trait, and a Draft struct.
...
// trait doesn't have any methods, and we'll start by defining just the Draft state because that is
// the state we want a post bo start in.
//
