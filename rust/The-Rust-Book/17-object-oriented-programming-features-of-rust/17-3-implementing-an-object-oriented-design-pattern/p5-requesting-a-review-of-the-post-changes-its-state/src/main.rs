...
//
// We add the request_review method to the State trait; all types that implements the trait will
// now need to implement the request_review method. Note that rather than having self, &self, or
// &mut self as the first parameter of the method, we have self: Box<Self>. This syntax means the
// method is only valid when called on a Box holding the type. This syntax takes ownership of
...
// can't use the old state value after we're transformed it into a new state.
//
...
// implements the requests_review method but doesn't do any transformations. Rather, it returns
// itself, because when we request a review on a post already in PendingReview state, it should
// stay in the PendingReview state.
//
// Now we can start sweeing the advantes of the state pattern: the request_review method on Post is
