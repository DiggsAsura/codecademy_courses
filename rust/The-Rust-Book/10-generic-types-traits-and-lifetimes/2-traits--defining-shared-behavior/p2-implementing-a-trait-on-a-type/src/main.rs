...
// this was supposed to be a lib.rs file, but i changed it up to work in a normal main.rs file
mod listing_10_13 {
    pub trait Summary {
...
// 10-13: Implementing the Summary trait on the NewsArticle and Tweet types
...
use listing_10_13::{Summary, Tweet};

fn main() {
    let tweet = Tweet {
        username: String::from("horse_ebooks"),
        content: String::from(
            "of course, as you probably already know, peopele",
        ),
        reply: false,
        retweet: false,
    };
...
// implement it on their own type. One restriction to note is that we can implement a trait on a
// type only if at least one of the trait or the type is local to our crate. For example, we can
// implement standard library traits like Display on a custom type like Tweet as part of our
...
// Display trait on Vec<T> within our aggreagor crate, because Display and Vec<T> are both defined
...
// type is not present. This rule ensures that other people's code can't break your code and vica
