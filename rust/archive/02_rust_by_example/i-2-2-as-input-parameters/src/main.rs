 ...
 * ambiguity is not allows when writing functions. When taking a closure as an input parameter, the
 * closure's complete type must be annotated using one of a few traits, and they're determined by
 * what the closure does with captured value. In order of decreasing restriction, they are:
 *
 ...
 * capture by &T, &mutT, or T, but the compiler will ultimately choose based on how the captured
...
fn main() {
    ...
    // Capture 2 variables: 'greeting' by reference and 'farevwell' by value.
    let diary = || {
        // 'greeing' is by reference: requires 'Fn'.
        println!("I said {}.", greeting);

        // Mutations forces 'farewell' to be captured by 
...
// Ok so here was a lot of greek stuff for me, which I'm not really ready to digest I think. Maybe
// later! 
