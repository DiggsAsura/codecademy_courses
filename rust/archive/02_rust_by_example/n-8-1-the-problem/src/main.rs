 ...
 *
 * In the example below, the Contains trait allows the use of the genric types A and B. The trait
 * is then implemented for the Container type, specifying i32 for A and B so it can be used with fn
 * difference().
 *
 * Because Contains is generic, we are forced to explicitly state all of the generic types for fn
 * difference(). In practice, we want a way to express that A and B are determined by the input c.
