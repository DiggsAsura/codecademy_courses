 ...
 * Path: posix::Path, for UNIX like systems, and windows::Path, for Windows. The prelude exports
 * the appropriate platform specific Path variant. 
 *
 ...
 * is similar to that of str and String: a PathBuf can be mutated in-place, and can be dereferenced
 * to a Path. 
 *
 * Note that a Path is not internally represented as an UTF-8 string, but instead is stored as a
 * vector of bytes (Vec<u8>). Therefore, converting a Path to a &str is not free and may fail (an
 * Option is returned). */
