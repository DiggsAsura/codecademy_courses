...
'''
You've successfully authenticated, Nice. Now only logged in users can see our
zesty recipe! But now that you're done you don't want to remain logged in. Let's
enable the logout feature to protect the recipe.
...
Awesome! We have our logout() function set up to call flask-login's logout_user()
function to log out the user. Then we redirect the user to go back to the index
page. Let's implement a logout link in our HTML to trigger the logout code to run.
...
When a user clicks the logout link, they'll call the logout() function we just
created.
