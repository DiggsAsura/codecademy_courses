...
# So the big Strings Project. 

# You've recently been hired as a cashier at the local sewing hobby shop, Thread Shed. Some of your daily responsibilities
# involve tallying the number of sales during the day, calculating the total amount of money made, and keeping track of the
# names of the customers. 
#
# Unfortunately, the Thread Shed has an extremly outdated register system and stores all of the transaction information
...
# is all recorded in this same string. Your task is to use your Python skills to iterate through this string and clean
...
# Excited for this one! In fact, more and more excited about coming back to programming python. Want to learn it now!

# Tasks:

# 1. First, take a minute to inspect the string daily_sales in the code editor. How is each transaction stored? 
#    How is each piece of data within the transaction stored? 
#    Start thinking about how we can split this string into its individual pieces of data. 
#    NAME;,;AMOUNT;,;PRODUCT;,;DATE, 
#    NAME;,;AMOUNT;,;PRODUCT;,;DATE,
# 
# 2. It looks like each transaction is separated from the next transaction by a , and then each piece of data 
#    within a transaction is separated by the artifact ;,;.
#    If we want to split up daily_sales into a list of individual transactions, we are going to want to split
#    by , but first, we need to replace the artifact ;,; to something without a comma, so we don't split any
#    transactions themselves.
#    Replace all the instances of ;,; in daily sales with some other character and save the result to 
...
#
# 6. Now, iterate through daily_transactions (remember, this is a list of strings currently), and for each 
#    transaction, split the string around whatever character you replaced the ;,; artifacts with in Step 2.
...
#    transactions_clean. Now iterate through daily_transactions_split and for each transaction iterate through the
...
#
# 10. Create three empty lists. customers, sales and thread_sold.
#     We are going to collect the individual data points for each transactions in these lists.
# 
...
# PHEWWW! That was one big project! I had to use some assistance from about halfway through, but everything makes sense. I 
# do feel i get into the groove a bit more every day! 
# Only gonna be one commit today i beleive, but this one was big. 
