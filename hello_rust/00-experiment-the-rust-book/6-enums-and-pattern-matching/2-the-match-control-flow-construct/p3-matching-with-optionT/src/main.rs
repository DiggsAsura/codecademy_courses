 ...
 *
 * In the previous section, we wanted to get the innter T value of the Some case when using
 ...
 *
 * Let's say we want to wirte a function that takes an Option<i32> and, if there's a value inside,
...
fn main() {
    ...
    println!("five: {:?}, six: {:?}, none: {:?}", five.unwrap(), six.unwrap(), none); // UNWRAP <3
}
 ...
 *
 * Now let's consider the second call of plus_one, where x is None. We enter the match and compare
 * the first arm: 
 *
 *    None => None,
 *
