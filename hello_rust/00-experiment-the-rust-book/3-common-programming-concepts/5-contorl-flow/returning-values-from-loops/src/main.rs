 ...
 *
 * One of the uses of a loop is to retry an operation you know might fail, such as checking wheter
...
fn main() {
    ...
    let result = loop {
        ...
        if counter == 10 {
            break counter * 20      // Works with and without ; meaning it can be a 
                                    // statement AND an expression? break is an expression 
        }
    };
 ...
 * we use the break keyword with the value counter * 20. After the loop, we use a semicolon to end
 * the statement that assigns the value to result. Finally, we print the value in result, which in
 * case is 200. */

/* Try to remember i can break counter * 20; */
