 ...
 * work without annotation. This allows captureing to flexibly adapt to the use case, sometimes
...
fn main() {
    ...
    // A closure to print 'color' which immiately borrows ('&') 'color' and stores the borrow and
    // closure in the 'print' variable. It will remain borrowed until 'print' is used the last
    // time. 
    //
    ...
    // mutates the closure which require a 'mut'. 
    let mut inc = || {
        count += 1;
        println!("'count': {}", count);
    };
    ...
    // leaving the original untouched. A non-copy must move and so 'moveable' immiately moves into
    // the closure.
    let consume = || {
    ...
    // immutably, hence _haystack_ is still available and uncommenting above line will not cause an
    // error. 
}
