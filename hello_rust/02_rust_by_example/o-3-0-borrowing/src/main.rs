 ...
 * Rust uses a borrowing mechanism. Instead of passing objects by value (T), objects can be passed
 * by reference (&T). 
 *
...
fn main() {
    ...
    // Borrow the contents of the box. Ownership is not taken, so the contents can be borrowed
    // again.
