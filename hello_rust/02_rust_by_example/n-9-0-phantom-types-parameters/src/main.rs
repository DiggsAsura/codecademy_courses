 ...
 *
 * In the following example, we combine std::marker::PahntomData with the phantom type parameter
...
// A phantom type struct which is generic over 'A' with hidden paramter 'B'.
#[derive(PartialEq)]  // Allow equality test for this type.
struct PhantomStrcut<A, B> { first: A, phantom: PhantomData<B> }

// Note: Storate is allocated for generic type 'A', but not for 'B'.
