...
fn main() {
    ...
    println!("Unused characters: {}", trimmed_str);
 ...
 * a similar &str so it's best to use the form that is the most convenient to write. Similarly
 * there are multiple ways to write byte string literals, which all result in &[u8; N]
 *
 * Generally special character are escaped with a backslash character: \. This way you can add any
...
/* Sometimes there are just too many characters that need to be escaped it's just much more
...
/* Want a string that's not UTF-8? (Remember, str and STring must be valid UTF-8). Or maybe you
 * want an array of bytes thta's mostly text? Byte strings to the rescue! */
...
fn ex4() {
    ...
    match str::from_utf8(shift_jis) {
        Ok(my_str)  => println!("Conversion succesful: {}", my_str),
