 ...
 * determined by where it is declared. As a result, the borrow is valid as long as it ends before
 * the lender is destroyed. However, the scope of the borrow is determined by where the reference
 * is used. 
 *
...
// 'i' has the longest lifetime because its scope entirely encloses both 'borrow' and 'borrow2'.
