...
fn main() {
    ...
    let reference: &i128 = &4;

    match reference {
        // If 'reference' is pattern matched against '&val', it results in comparison like
        // '&i32'
        // '&val'
    ...
    // because the right side was already a reference. This is not a reference because the right
    // side is not one.

    let _not_a_reference = 3;

    // Rust provides 'ref' for exactly this purpose. It modifies the assignement so that a
    // reference is created for the element; this reference is assigned. 
    let ref _is_a_reference = 3;  // but is it the same as just writing &3? 
    ...
    match value {
        ref k => println!("Got a reference to a value: {:?}", k),
    }
