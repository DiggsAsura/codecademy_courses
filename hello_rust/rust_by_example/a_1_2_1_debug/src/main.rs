 ...
 *
 * The fmt::Debug trait makes this very straightforward. All types can derive (automatically
 * create) the fmt::Debug implementation. This is not true for fmt::Display which must
...
fn main() {
    ...
    // No real answer for that in the article lol.

    // So fmt::Debug definitely makes this printable but sacrifices some elegance. Rust
    // also provides "pretty printing" with {:#?}
    #[derive(Debug)]
    struct Person<'a> {
        name: &'a str,
        age: u8
    ...
    let name = "Kenneth";
    let age = 38;
    let kenneth = Person { name, age };

    // Pretty print
    println!("{:#?}", kenneth);
}

// One can manually implement fmt::Dispaly to control the display.
